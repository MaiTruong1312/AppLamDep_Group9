...
class _ChatScreenState extends State<ChatScreen> {
  ...
  // SỬA LỖI: Đồng nhất key 'imageUrl' để hiển thị ảnh
  Future<void> _saveMessage({required String text, required bool isUser, List<Service>? suggestions}) async {
    await _messageRef.add({
      ...
      'suggestions': suggestions?.map((s) => {
        'name': s.name,
        'imageUrl': s.imageUrl, // Đồng nhất key imageUrl
      }).toList(),
    });
    _scrollToBottom();
  }
  ...
  Widget _buildInputIcon(IconData icon) => Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: Icon(icon, color: const Color(0xFF4B5563), size: 24));
}